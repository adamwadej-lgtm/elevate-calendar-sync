...
type DailySchedule = Record<number, TimeRange | null>; // 0-6 (Sun-Sat)

type Profile = {
  id: string;
  name: string;
  startDate?: string; // Individual anchor Sunday
  availability: {
    weekA: DailySchedule;
    weekB: DailySchedule;
  };
...
const getIntersection = (ranges: TimeRange[]): TimeRange | null => {
  ...
  return {
    ...
    end: `${resEndH.toString().padStart(2, '0')}:${resEndM.toString().padStart(2, '0')}`
  };
};
...
export default function App() {
  ...
  const [profiles, setProfiles] = useState<Profile[]>(() => {
    const saved = localStorage.getItem('team-sync-profiles');
    return saved ? JSON.parse(saved) : [];
  });
  
  const [events, setEvents] = useState<AppEvent[]>(() => {
    const saved = localStorage.getItem('team-sync-events');
    return saved ? JSON.parse(saved) : [];
  });
  ...
  useEffect(() => {
    localStorage.setItem('team-sync-profiles', JSON.stringify(profiles));
  }, [profiles]);

  useEffect(() => {
    localStorage.setItem('team-sync-events', JSON.stringify(events));
  }, [events]);
  ...
  // Persist local selection to Event
  useEffect(() => {
    if (activeEventId) {
  ...
  const handleBulkDeleteEvents = () => {
    ...
    setEvents(prev => prev.filter(ev => !selectedEventIds.includes(ev.id)));
  ...
  const handleSaveProfile = (profile: Profile) => {
    // Ensure startDate is always explicit and normalized to Sunday
              ...
              >
                ...
                <div className="p-6 border-t border-gray-100 bg-gray-50 flex justify-between gap-3">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button 
                      disabled={!editingEvent.title || !editingEvent.planningEndDate}
                      onClick={handleSaveEvent} 
                      className="px-8 py-2 rounded-xl bg-brand-orange text-white font-bold shadow-lg hover:bg-orange-600 transition-all disabled:opacity-50"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-gray-50 border-gray-200 border rounded-xl px-4 py-3 focus:ring-2 focus:ring-brand-orange outline-none"
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      className="w-full bg-gray-50 border-gray-200 border rounded-xl px-4 py-3 focus:ring-2 focus:ring-brand-orange outline-none bg-none appearance-none cursor-pointer font-bold"
                      value={editingProfile.startDate || DEFAULT_SUNDAY} 
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    <WeekScheduleEditor 
                      title="Week One" 
                      subtitle="Schedule"
                      schedule={editingProfile.availability.weekA} 
                      onChange={(s) => setEditingProfile({
                        ...editingProfile,
                    ...
                    <WeekScheduleEditor 
                      title="Week Two" 
                      subtitle="Schedule"
                      schedule={editingProfile.availability.weekB} 
                      onChange={(s) => setEditingProfile({
                        ...editingProfile,
              ...
              <div className="p-6 bg-gray-50 border-t border-gray-100 flex justify-between items-center gap-3">
                ...
                <div className="flex gap-3">
                  ...
                  <button 
                    ...
                    className="px-8 py-2.5 rounded-xl font-bold bg-brand-orange text-white shadow-sm hover:bg-orange-600 transition-all disabled:opacity-50 flex items-center gap-2"
                  >
                ...
                {isActive && (
                  <div className="flex items-center gap-2 bg-gray-100 p-1 rounded-xl">
                    <input 
                      ...
                      onChange={(e) => {
                        onChange({ ...schedule, [day.value]: { ...schedule[day.value]!, start: e.target.value } });
                      }}
                    />
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        onChange({ ...schedule, [day.value]: { ...schedule[day.value]!, end: e.target.value } });
                      }}
                    />
                  </div>
                )}
